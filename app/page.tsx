...
export default function Home() {
  ...
  if (finished) return <main className="result-page"><div className="topbar"><div className="brand"><span className="brand-mark">〽</span><span>ニッポン地理テスト</span></div><button className="back-button" onClick={() => setStarted(false)}>トップへ戻る</button></div><section className="result-card"><p className="eyebrow">YOUR RESULT</p><h1>おつかれさまでした！</h1><div className="score-ring"><span>{score}</span><small>/ {questions.length} 問正解</small></div><p className="result-message">{score >= 17 ? "すごい！日本地理マスターです。" : score >= 12 ? "いい調子！旅の引き出しが増えました。" : "ここから伸びしろたっぷり。もう一度挑戦！"}</p><div className="review-list"><div className="review-heading"><span>ANSWER REVIEW</span><b>回答の振り返り</b></div>{questions.map((item, index) => { const userAnswer = answers[index]; const isCorrect = userAnswer === item.answer; return <article className={`review-item ${isCorrect ? "correct" : "incorrect"}`} key={item.id}><div className="review-number">{String(index + 1).padStart(2, "0")}</div><div className="review-body"><div className="review-status"><span>{isCorrect ? "正解" : "不正解"}</span><small>{item.category}</small></div><h2>{item.prompt}</h2><p className="your-answer">あなたの回答：<b>{userAnswer === null || userAnswer === undefined ? "未回答" : item.choices[userAnswer]}</b></p>{!isCorrect && <p className="correct-answer">正解：<b>{item.choices[item.answer]}</b></p>}<p className="explanation">{item.explanation}</p></div></article>; })}</div><button className="primary retry" onClick={start}>もう一度挑戦する <span>↻</span></button></section></main>;
